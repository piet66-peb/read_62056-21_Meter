...
#h               This version of script read_meter.bash sends the data to Z-Way virtual 
#h               device(s).
#h
#h Installation: 
#h Project:      
#h Usage:        1. start manually:
#h                 <path>/<bash> >/dev/null 2>&1 &
#h               2. run with cron:
#h                 crontab -e
#h                 and enter:
#h                   @reboot <path>/<bash> >/dev/null 2>&1
#h Result:       
#h Examples:     
#h Outline:      
...
#h Version:      V3.1.0 2023-07-24/peb
...
MODULE='read_meter_ZWay';
VERSION='V3.1.0'
WRITTEN='2023-07-24/peb'
    ...
    do
        ...
        do
            ...
            case "$line" in 
                ...
                #b data line: strip and send OBIS value, if ZWAY_device is defined 
                #-----------------------------------------------------------------
                *\(*)
                    if [ "$IEC_62056_21_MODE" == "D" ]
                    then
                    ...
                    if [ "$STORE_ZWAY" == true ]
                    then
                        obis_code="${line%\(*}"     #remove last part from '(' on
                        #echo $obis_code
                        key="${obis_code//./}"      #remove all dots
                        #echo $key
                        zway_device=${ZWAY_devices[$key]}
                
                        if [ "$zway_device" != "" ]
                        then
                            #echo $zway_device
                            a="${line#*\(}"         #remove first part till '('
                            a="${a%\**}"            #remove last part from '*' on
                            value="${a%\)*}"        #remove last part from ')' on
                            #echo $value
                            echo ">> ./store_zway.bash $zway_device $value"
                            ./store_zway.bash $zway_device $value &
                        fi
                    fi
                    ;;
